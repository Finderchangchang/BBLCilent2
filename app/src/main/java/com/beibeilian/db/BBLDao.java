...
import com.beibeilian.model.VIPOrder;
...
public class BBLDao extends SQLiteOpenHelper {
	...
	private String paysql = "create table if not exists price_power(id integer primary key autoincrement,visit varchar(10),anlian varchar(10),relation varchar(10),gift varchar(10),marriage varchar(10),ball varchar(2),chat varchar(2),price_15 varchar(10),price_30 varchar(10),price_180 varchar(10),price_360 varchar(10),modtime varchar(20))";
	...
	private String msgzh="你好,我喜欢你这个类型的，我们可以相互了解一下吗";
	...
	public List<MessageList> queryMessageList(String pagenumber, String username) {
		if (pagenumber.equals("0")) {
			...
			pagenumber = String.valueOf(Integer.parseInt(pagenumber) * com.beibeilian.util.PublicConstant.PageSize);
		}
		...
		String sql = "select userout,userin,content,time from privateletter W where time=(select MAX(time) from privateletter where userout=W.userout and userin=W.userin or userout=w.userin and userin=W.userout) and delstate='0' order by time desc  limit "
				+ pagenumber + "," + com.beibeilian.util.PublicConstant.PageSize + "";
		...
		while (cursor.moveToNext()) {
			...
			try {
				messageList.setFromcontent(new String(content,"UTF-8"));
			} catch (Exception e) {
				e.printStackTrace();
			}*/
	...
	public List<HomeMessage> queryChatMessageList(String fromuser, String touser, String pagenumber) {
		try {
			...
			String sql = "select userout,content,time,sendstate,imid from privateletter where (userout='" + fromuser
					...
					+ "') order by time desc limit " + pagenumber + "," + com.beibeilian.util.PublicConstant.PageSize
					+ "";
			db = this.getReadableDatabase();
			Cursor cursor = db.rawQuery(sql, null);
	...
	public boolean queryChatISSend3(String fromuser) {
		try {
			int count=0;
			String sql = "select userout,content from privateletter where userout='" + fromuser+ "' and content!='"+msgzh+"' limit 0,10";
			db = this.getReadableDatabase();
			Cursor cursor = db.rawQuery(sql, null);
			while (cursor.moveToNext()) {
				String outuser = cursor.getString(0);
